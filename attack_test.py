...
def main():
    ...
    # 指定卸载文件夹
    offload_folder = f"outputs/{fname}/offload"
    ...
    """改"""
    output_dir = f"outputs_data/{fname}"
    ...
    if args.use_greedy:
        ...
        for sentence in tqdm(lines):
            try:
                ...
                # 手动添加起始标记
                input_ids = torch.cat([torch.tensor([[tokenizer.bos_token_id]]), encoded_input.input_ids], dim=-1).to(
                    "cuda")

                # 调用 generate 方法，并传递起始标记
        ...
        for temp in np.arange(0.05, 1.05, 0.05):
            ...
            for sentence in tqdm(lines):
                try:
                    ...
                    # 手动添加起始标记
                    input_ids = torch.cat([torch.tensor([[tokenizer.bos_token_id]]), encoded_input.input_ids],
                                          dim=-1).to(
                        "cuda")
                ...
                results["prompt"] = [line.strip() for line in prompts]
                results["output"] = outputs
                results.to_csv(f"outputs_data/{fname}/output_temp_{temp}.csv")
